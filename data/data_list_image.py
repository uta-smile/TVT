...
def make_dataset(image_list, labels):
    if labels:
      ...
      if len(image_list[0].split()) > 2:
        images = [(val.split()[0][1:], np.array([int(la) for la in val.split()[1:]])) for val in image_list]
      else:
        images = [(val.split()[0][1:], int(val.split()[1])) for val in image_list]
    return images
